...
**Spec (the contract, wins on any conflict):** `docs/2026-09-20-script-drilldown-spec.md`
...
```
 ...
 │    1 Test-TrustedProgram      (D9 trust-first; hit ⇒ today's path entry)           │
...
| ON: **expansion OK** | wrapperResults = `[pseudo-wrapper(IsTerminal,SkipRewalk), stmt1..stmtN(SkipRewalk,...)]`; real outer suppressed ⇒ `[1]`=wrapper, `[2..n]`=statements | returns `[wrapper entry, stmt1..stmtN]` with `ParentCommand=$trimmed`; the segment itself is dropped by the existing `parentTexts` filter |
...
**Why `SkipRewalk`:** after adding each inner result, `Get-AstCommands` re-walks it
(`Get-PowerShellCommands($innerCmd)`, ~L1750–1775). Re-walking a wrapper text or a
`pwsh -File z.ps1` statement re-enters the engine; the HT loop-check would then produce spurious
`recursive …` entries. Mark every engine-emitted entry `SkipRewalk=$true` and skip the re-walk
when present. (Without this, a clean script containing `pwsh -File helper.ps1` false-asks.)
...
4. `runners`: array of strings; EVERY entry must be in the implemented set `{ 'powershell' }` —
   ...
   Omitted ⇒ `@('powershell')` (all implemented matchers armed).
...
#   INPUT : the command segment text (already &-stripped / full-path-normalized
#           by the caller when coming from SITE B; SITE A passes the wrapper
#           extent text and may override ScriptPath with its parsed element value)
...
$script:ScriptRunnerMatchers = @(
    @{
        ...
        Match  = {
            param([string]$Text)
            if ($Text -match '(?i)^(?:\.?\\)?(?:pwsh|powershell)(?:\.exe)?\s+.*?-File\s+(?:"([^"]+)"|''([^'']+)''|(\S+))') {
...
function Expand-ScriptFile {
    ...
    # Returns an array of parser-level entries. Entry kinds:
    #
    # KIND              Properties (beyond the standard CommandText/Domain/IsPipeline/ParentCommand)
    # ----              ----------
    # TRUSTED           IsTerminal=$true (SITE A)            — today's path entry; Resolver stamps
    #                                                      trusted_program. SITE B: plain entry.
...
│              if $hit → return TRUSTED entry (path as written, Domain 'powershell',
│              ParentCommand $WrapperText). FILE IS NEVER READ. (D9: trust short-circuits
│              before expansion; today's behavior for every currently-trusted path.)
│
...
│
├─ 8. EMPTY     if $stmts.Count -eq 0 →
│                 FAILURE 'script contains no classifiable statements (fail-closed): <ScriptPath>'
│               (Covers parse-failure too: Get-PowerShellCommands returns @() on parse errors.)
│
├─ 9. EMIT      $out = @()
│               foreach $s in $stmts:
│                 $text = $s.CommandText
...
| `SkipRewalk` | ALL engine-emitted entries | do not re-walk this entry in `Get-AstCommands` | `Get-AstCommands` |
...
1. `AddResult` gains an optional `[int]$LineNumber = 0` parameter, stored when non-zero. The
   CommandAst loop passes `$cmd.Extent.StartLineNumber`. (Walker-level statements inside
   function bodies get their true lines via the existing ScriptBlockAst recursion, F3.)
...
| I7 | Dynamic constructs fail closed (R13) | `& $p`, `iex $x`, bare `.\x.ps1` (no operator), `Start-Process` hiding: no matcher/D13 hit ⇒ plain statement ⇒ unknown ⇒ ask. Documented v1 over-ask. |
...
| I9 | Re-entrant extraction harmless | §7 idempotence + `SkipRewalk`. |
...
test/config/script-drilldown/
    ...
    Run-Tests.ps1          preflights + TestRunner invocations (mirrors
                           test/config/trusted-programs-regex/Run-Tests.ps1 house style)
    ...
    scripts/
        ...
        loop-b.ps1             . .\loop-a.ps1 ; Write-Host b        (loop!)
...
| SD-Ask-Loop | `pwsh -File scripts\loop-a.ps1` | ask | reason-contains `recursive script invocation detected` |
...
SD-Ask-UntrustedUnknownStmt): unknown statements render
`script <basename> contains unknown command: <statement> (line N)`.
